...
# guarantee, and the first person to unpack one sees a folder called Contents
# and has no reason to know which part is the program.
#
...
if command -v tiffutil >/dev/null 2>&1 && \
   [ -f assets/brand/dmg-background@2x.png ]; then
    if tiffutil -cathidpicheck assets/brand/dmg-background.png \
                ...
                -out /tmp/sn-dmg-background.tiff >/dev/null 2>&1; then
        BG=/tmp/sn-dmg-background.tiff
    fi
fi
