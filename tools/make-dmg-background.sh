...
draw() {
    ...
    # No S. Tarr on the background: the icon the window is about to draw at
    # the left of that arrow is S. Tarr, and a second one in the same window
    # reads as a second thing to drag.
    ...
    "$IM" -size "${w}x${h}" "xc:#0c1408" \
        ...
        -annotate "+$((214 * scale))+$((104 * scale))" "BENCsnip" \
        -fill "#6f8a5c" -pointsize $((15 * scale)) \
        -annotate "+$((216 * scale))+$((136 * scale))" "a video editor" \
        ...
        -gravity north \
        -annotate "+0+$((330 * scale))" "drag BENCsnip to Applications" \
        "$out"
