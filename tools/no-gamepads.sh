...
#
# This is a video editor. It has never read a joystick and it never will, and
...
if grep -q 'BENCsnip: no gamepads' "$J"; then
...
awk -v a="$ANCHOR" '
    ...
    { print }
    found && !done && $0 == "{" {
        print "    /* BENCsnip: no gamepads. See tools/no-gamepads.sh."
        print "     *"
